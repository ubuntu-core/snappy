...
func (s *vitalitySuite) testConfigureVitalityWithQuotaGroup(c *C, assertStrings [][]string) {
...
func (s *vitalitySuite) testConfigureVitalityWithQuotaGroupOldSystemd(c *C) {
	...
	s.testConfigureVitalityWithQuotaGroup(c, [][]string{
		{"daemon-reload"},
	})
}
...
func (s *vitalitySuite) testConfigureVitalityWithQuotaGroupDaemonReload(c *C, assertStrings [][]string) {
	si := &snap.SideInfo{RealName: "test-snap", Revision: snap.R(1)}
	snaptest.MockSnap(c, mockSnapWithService, si)
	s.state.Lock()
	snapstate.Set(s.state, "test-snap", &snapstate.SnapState{
...
func (s *vitalitySuite) TestConfigureVitalityWithQuotaGroupDaemonReloadOldSystemd(c *C) {
	...
	s.testConfigureVitalityWithQuotaGroupDaemonReload(c, [][]string{
		{"daemon-reload"},
	})
}
