...
apps:
 app2:
  command: foo
  plugs: [hardware-control]
`

var _ = Suite(&HardwareControlInterfaceSuite{
	iface: builtin.MustInterface("hardware-control"),
})
...
func (s *HardwareControlInterfaceSuite) SetUpTest(c *C) {
	...
	s.plugInfo = plugSnap.Plugs["hardware-control"]
	s.plug = interfaces.NewConnectedPlug(s.plugInfo, nil, nil)
}

func (s *HardwareControlInterfaceSuite) TestName(c *C) {
	c.Assert(s.iface.Name(), Equals, "hardware-control")
}
...
func (s *HardwareControlInterfaceSuite) TestUsedSecuritySystems(c *C) {
